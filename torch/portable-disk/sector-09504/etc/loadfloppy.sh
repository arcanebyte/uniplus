if yawn "LOADFLOPPY" "Put disc in drive. Ready ?"
then
while :
do
	...
	case $fstype in
		0)	# bad format or floppy not loaded
			if yawn "LOADFLOPPY" "Please check that the floppy to be read has been correctly inserted into the floppy disc drive. Try Again ?"
			then
				continue
			else
				exit
		...
		1)	# non formatted
			if yawn "LOADFLOPPY" "Please check that the floppy to be read has been formatted. Try Again ?"
			then
				continue
			else
				exit
			...
			;;
		2)	# Empty disc
			if yawn "LOADFLOPPY" "Please check that the floppy to be read has information on it. Try Again ?"
			then
				continue
			else
				exit
