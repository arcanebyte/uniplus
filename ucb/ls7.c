 ...
 *
 * Mod by dgc@tek, 3/83:	print both user and group for -l
 *
 * Lisa (UniPlus+ System V), from 2.9BSD's net/src/cmd/ls.c, installed as
 * ls7 beside System V's ls: directories are read through ndir.c (System V
 * has no opendir); there are no symbolic links, so -L does nothing and
 * lstat is stat; the tab test uses termio (TAB3) instead of sgtty (XTABS).
 */
	...
	char *argv[];
{
	...
	if (isatty(1)) {
		...
		qflg = Cflg = 1;
		if (ioctl(1, TCGETA, (char *)&tbuf) < 0 ||
		    (tbuf.c_oflag & TABDLY) != TAB3)
			usetabs = 1;
	} else
		usetabs = 1;
	while (argc > 0 && **argv == '-') {
