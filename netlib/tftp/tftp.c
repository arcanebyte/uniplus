/*	tftp.c	4.2	82/08/17	*/
/*
 * Lisa (UniPlus+ with unix.net), ported from 2.9BSD's netser/tftp:
 *  - Replies come from the server's transfer port, not port 69 (RFC 783
 *    and every server since), so after the first reply everything goes
 *    to that port; each transfer starts again at the server's port.
 *  - signal() instead of sigset(); the alarm handler re-arms itself.
 *  - The request's mode string is measured, not assumed to be as long
 *    as "netascii".
 */
...
extern	char mode[];
extern	int port;		/* the server's port for requests */
int	f;
int	trace;
int	verbose;
int	connected;
...
again:
		...
		if (from.sin_addr.s_addr != sin.sin_addr.s_addr)
			goto again;
		sin.sin_port = from.sin_port;	/* the server's transfer port */
