...
again:
	...
	while (dp = readdir(etc)) {
		...
		for (n = cc / sizeof (struct whoent); n > 0; n--) {
			...
			mp->myxutmp = we->we_utmp; mp->myidle = we->we_idle;
	...
	width = 0;
	for (i = 0; i < nusers; i++) {
		int j = strlen(mp->myhost) + 1 + strlen(mp->myxutmp.ut_line);
		if (j > width)
			width = j;
		mp++;
	}
	...
	for (i = 0; i < nusers; i++) {
		char buf[22];
		sprintf(buf, "%s:%s", mp->myhost, mp->myxutmp.ut_line);
		printf("%-8.8s %-*s %.12s",
		   mp->myxutmp.ut_name,
		   width,
		   buf,
	...
	struct myutmp *u1, *u2;
{
	...
	rc = strncmp(u1->myxutmp.ut_name, u2->myxutmp.ut_name, 8);
	if (rc)
		return (rc);
	rc = strncmp(u1->myhost, u2->myhost, 8);
	if (rc)
		return (rc);
	return (strncmp(u1->myxutmp.ut_line, u2->myxutmp.ut_line, 8));
}
