	...
	struct mbuf *m0;
{
	...
	if (((struct ip *)ui)->ip_len != ulen) {       /* ip_input has subtracted ip hlen */
		if (ulen > ((struct ip *)ui)->ip_len) {
			udpstat.udps_badlen++;
			goto bad;
		}
		m_adj(m, -(((struct ip *)ui)->ip_len - ulen));
	}
	...
	if (inp == 0) {
		...
		broadcastaddr = if_makeaddr(in_netof(ui->ui_dst), INADDR_ANY);
	...
	struct mbuf *m0;
{
	...
	ui->ui_ulen = len + sizeof(struct udphdr);
#if vax || pdp11
