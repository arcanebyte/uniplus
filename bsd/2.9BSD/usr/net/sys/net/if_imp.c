...
#define IMPTIMEOUT 15*hz		/* how often to try to init IMP */
					/* after IMP ready line drops */
	...
	int unit;
{
	...
	if ((*sc->imp_cb.ic_init)(unit) == 0) {
		...
		timeout(impinit, (caddr_t) unit, IMPTIMEOUT); /* try again */
		splx(s);
		return;
	}
	...
	register struct mbuf *m;
{
	...
	if(m == 0) {	/* kludge to indicate ready line just dropped */
		impmsg(sc, "ready line dropped");
		impdown(sc);
		timeout(impinit, (caddr_t)unit, IMPTIMEOUT);
		  /* call impinit to bring it back up in a little while */
		return;	/* m is a fake mbuf, so don't need to free it */
	}
	...
	case IMPTYPE_NOOP:
		...
		if (sin->sin_addr.s_host != ip->il_host ||
		    UCHAR(sin->sin_addr.s_impno) != ntohs(ip->il_imp)) {
			sc->imp_if.if_host[0] =
				sin->sin_addr.s_host = ip->il_host;
			sin->sin_addr.s_impno = ntohs(ip->il_imp);
...
/*#ifdef never*/
		int s = splnet();
		impnotify(UCHAR(ip->il_mtype), ip, hostlookup(addr));
		splx(s);
/*#endif*/
		goto rawlinkin;
	}
	...
	default:
	rawlinkin:
		impproto.sp_protocol = UCHAR(ip->il_link);
		sin = (struct sockaddr_in *)&sc->imp_if.if_addr;
	...
	struct host *hp;
{
	...
	in.s_imp = cp->dl_imp;
	if (UCHAR(cp->dl_link) != IMPLINK_IP)
	...
	struct sockaddr *dst;
{
	...
	imp->il_length = htons((u_short) len << 3); /* BRL 3/11/83 */
	imp->il_link = dlink;
