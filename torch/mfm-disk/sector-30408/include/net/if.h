/*	@(#)if.h 1.1 86/02/03 SMI; from UCB 6.2 83/08/28        */
/*	@(#)if.h	2.1 86/04/15 NFSSRC */
...
/*
 * Interface download request structure (overlays ifr_data)
 * N.B.:  sizeof(struct ifload) must be less than sizeof(ifr_ifru)
 *	  in the ifreq structure above.
 */
struct ifload {
	caddr_t	ifl_saddr;
	caddr_t	ifl_taddr;
