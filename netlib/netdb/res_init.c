...
/*
 * Lisa: from 4.3BSD.  Without a "nameserver" line in /etc/resolv.conf
 * there is no name server (nscount 0) rather than one on this host,
 * since the Lisa runs no named; res_send() then fails at once and
 * gethostbyname() uses /etc/hosts.  With one nameserver line 4.3BSD kept
 * nscount at its default of 1, which is the same thing.
 */
