...
/*
 * Lisa: from 4.3BSD.  System V's sprintf() returns a count, not the
 * buffer, and the header's unused bits are cleared (now the AD and CD
 * flags of DNSSEC).
 */
	...
	int buflen;		/* size of buffer */
{
	...
	hp->rcode = NOERROR;
	hp->unused = 0;		/* Lisa: 4.3BSD sent whatever was in buf */
	hp->qdcount = 0;
	hp->ancount = 0;
	hp->nscount = 0;
	hp->arcount = 0;
