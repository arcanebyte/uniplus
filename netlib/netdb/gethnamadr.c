...
/*
 * Lisa: 4.3BSD's named/gethostnamadr.c, renamed to fit 14 characters.
 *  - gethostbyname() and gethostbyaddr() use /etc/hosts whenever the
 *    name server gives no answer, not only when there is none (4.3BSD
 *    fell back only on ECONNREFUSED).  A site running named would serve
 *    its own hosts; the Lisa's server (slirp's, or one on the internet)
 *    doesn't know lisa or gateway.
 *  - A PTR answer ends the alias list, which 4.3BSD left holding the
 *    aliases of an earlier lookup.
 *  - _gethtbyname and _gethtbyaddr are renamed apart in netdb.h
 *    (7-character external names); host_addrs is htaddrs, as cc keeps
 *    8 characters of a name and host_addr is taken.
 */
