...
/*
 * Lisa: 4.1c's ruserpass() from 2.9BSD's net/common, for ftp.  Passwords
 * come from MACHhost=login,encrypted variables in the environment, then
 * from $HOME/.netrc, then are asked for, as in 4.1c.  Changes:
 * <sys/types.h> first (System V's utmp.h needs time_t); index() from
 * bsd.h; the static routines declared static before use; getutmp()
 * takes only the USER_PROCESS entry for the terminal, since a System V
 * utmp also keeps getty and dead entries; and the login name taken from
 * the environment is terminated.  The encryption key is left as 4.1c
 * made it: the terminal name and the host's first letter (sreverse()
 * copies the NUL first, so the login time drops out).
 */
#include <sys/types.h>
#include <stdio.h>
#include <utmp.h>
	...
	char *host, **aname, **apass;
{
	...
	if (*aname == 0) {
		*aname = malloc(comma - cp + 1);
		...
		(*aname)[comma - cp] = 0;	/* Lisa: was unterminated */
	} else
...
static	enblkclr(), enblknot(), nbssetkey(), blkencrypt();
static char	E[48];
...
char *sttyname;
{
	...
	while(fread(&utmpstr,1,sizeof utmpstr,fdutmp) == sizeof utmpstr)
		if(utmpstr.ut_type == USER_PROCESS &&
		    strcmp(utmpstr.ut_line,sttyname+5) == 0){
			fclose(fdutmp);
			return(&utmpstr);
		}
	fclose(fdutmp);
